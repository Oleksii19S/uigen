...
function FileTreeNode({ node, level }: FileTreeNodeProps) {
  ...
  return (
    <div>
      <div
        className={cn(
          "flex items-center gap-2 px-2 py-1.5 hover:bg-neutral-800 cursor-pointer text-sm transition-colors",
          selectedFile === node.path && "bg-blue-900/40 text-blue-400"
        )}
  ...
  if (!rootNode || !rootNode.children || rootNode.children.size === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-4 text-center">
        <Folder className="h-12 w-12 text-neutral-700 mb-3" />
        <p className="text-sm text-neutral-500">No files yet</p>
        <p className="text-xs text-neutral-600 mt-1">Files will appear here</p>
      </div>
    );
  }
